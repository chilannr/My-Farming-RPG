...
public class ApplyCharacterCustomisation : MonoBehaviour
{
    ...
    [SerializeField] private Texture2D adornmentsBaseTexture = null; // 装饰品基础纹理
    private Texture2D farmerBaseTexture; // 农民基础纹理

    // 创建的纹理
    [Header("用于动画的输出基础纹理")]
    [SerializeField] private Texture2D farmerBaseCustomised = null; // 定制的农民基础纹理
    [SerializeField] private Texture2D hairCustomised = null; // 定制的发型纹理
    [SerializeField] private Texture2D hatsCustomised = null; // 定制的帽子纹理
    private Texture2D farmerBaseShirtsUpdated; // 更新的农民衬衫纹理
    private Texture2D farmerBaseAdornmentsUpdated; // 更新的农民装饰品纹理
    private Texture2D selectedShirt; // 选择的衬衫纹理
    private Texture2D selectedAdornment; // 选择的装饰品纹理

    // 选择衬衫样式
    [Header("选择衬衫样式")]
    [Range(0, 1)]
    [SerializeField] private int inputShirtStyleNo = 0; // 输入衬衫样式编号

    // 选择发型样式
    [Header("选择发型样式")]
    [Range(0, 2)]
    [SerializeField] private int inputHairStyleNo = 0; // 输入发型样式编号

    // 选择帽子样式
    [Header("选择帽子样式")]
    [Range(0, 1)]
    [SerializeField] private int inputHatStyleNo = 0; // 输入帽子样式编号

    // 选择装饰品样式
    [Header("选择装饰品样式")]
    [Range(0, 2)]
    [SerializeField] private int inputAdornmentsStyleNo = 0; // 输入装饰品样式编号

    // 选择肤色类型
    [Header("选择肤色类型")]
    [Range(0, 3)]
    [SerializeField] private int inputSkinType = 0; // 输入肤色类型

    // 选择性别
    [Header("选择性别: 0=男性, 1=女性")]
    [Range(0, 1)]
    [SerializeField] private int inputSex = 0; // 输入性别

    // 选择头发颜色
    [SerializeField] private Color inputHairColor = Color.black; // 输入头发颜色

    // 选择裤子颜色
    [SerializeField] private Color inputTrouserColor = Color.blue; // 输入裤子颜色

    private Facing[,] bodyFacingArray; // 身体朝向数组
    private Vector2Int[,] bodyShirtOffsetArray; // 身体衬衫偏移数组
    private Vector2Int[,] bodyAdornmentsOffsetArray; // 身体装饰品偏移数组
    ...
    private int shirtStylesInSpriteWidth = 16; // 精灵宽度中的衬衫样式数量

    private int hairTextureWidth = 16; // 发型纹理宽度
    private int hairTextureHeight = 96; // 发型纹理高度
    private int hairStylesInSpriteWidth = 8; // 精灵宽度中的发型样式数量

    private int hatTextureWidth = 20; // 帽子纹理宽度
    private int hatTextureHeight = 80; // 帽子纹理高度
    private int hatStylesInSpriteWidth = 12; // 精灵宽度中的帽子样式数量

    private int adornmentsTextureWidth = 16; // 装饰品纹理宽度
    private int adornmentsTextureHeight = 32; // 装饰品纹理高度
    private int adornmentsStylesInSpriteWidth = 8; // 精灵宽度中的装饰品样式数量
    private int adornmentsSpriteWidth = 16; // 装饰品精灵宽度
    private int adornmentsSpriteHeight = 16; // 装饰品精灵高度

    private List<colorSwap> colorSwapList; // 颜色交换列表

    // 用于颜色替换的目标手臂颜色
    private Color32 armTargetColor1 = new Color32(77, 13, 13, 255); // 最深
    private Color32 armTargetColor2 = new Color32(138, 41, 41, 255); // 次深
    private Color32 armTargetColor3 = new Color32(172, 50, 50, 255); // 最浅

    // 用于颜色替换的目标肤色
    private Color32 skinTargetColor1 = new Color32(145, 117, 90, 255); // 最深
    private Color32 skinTargetColor2 = new Color32(204, 155, 108, 255); // 次深
    private Color32 skinTargetColor3 = new Color32(207, 166, 128, 255); // 次浅
    private Color32 skinTargetColor4 = new Color32(238, 195, 154, 255); // 最浅
